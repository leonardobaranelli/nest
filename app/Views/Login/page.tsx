...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    //   // Llamar a useUserVerifyQuery en un efecto secundario
    //   useEffect(() => {
    //     const userVerifyResponse = useUserVerifyQuery({
    //       email: userEmail,
    //       token: token,
    //     });
    //     // Haz algo con userVerifyResponse si es necesario
    //   }, [userEmail, token]);
  ...
  let authUrl;
  const handleLogGoogle = async (event: React.MouseEvent<HTMLButtonElement>) => {
    authUrl = await `${process.env.NEXT_PUBLIC_BACKEND_URL}/auth/google`;
    localStorage.setItem('authUrl', authUrl);
    window.location.href = authUrl;
  };

  useEffect(() => {
    const response = localStorage.getItem('authUrl');
    console.log(response)

    if (!isAuthenticated) dispatch(authenticateUserWithTokenAsync());
