...
const Usuarios = () => {
  ...
  return (
    <div>
          <div className="rounded-sm border border-stroke text-center bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      ...
      <div className="flex flex-col overflow-scroll overflow-y-auto h-[600px]">
        ...
        {user?.map((user, key) => (
          <div
            className={`grid grid-cols-3 sm:grid-cols-5 text-center border-b-2`}
            key={key}
          >
            ...
            <div className="flex items-center justify-center p-2.5 xl:p-5">
              <p className={`text ${user.rol === 'admin' ? 'text-red-500' : 'text-blue-500'} uppercase`}>{user.rol}</p>
            </div>
            ...
            <div className="flex items-center justify-center p-2.5 xl:p-5">
              ...
              {user.deletedAt ? (
                <p className="text-red-500">Eliminado el {new Date(user.deletedAt).toLocaleDateString()}</p>
              ) : (
              <button onClick={() => handleClick(user.id)} className="hover:text-primary">
                  <svg 
